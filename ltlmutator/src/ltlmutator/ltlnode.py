...
from .grammar.ltlListener import ltlListener
from antlr4 import CommonTokenStream, ParseTreeWalker
from antlr4 import ParseTreeWalker, CommonTokenStream, InputStream
from .grammar.ltlLexer import ltlLexer
from .grammar.ltlParser import ltlParser
from abc import ABC, abstractmethod


# Lazy-load ltltoeng: it's an optional dependency not included in this package.
# When available (e.g., in the full LTLTutor app), __to_english__() uses it
# for rich English translations. When unavailable, falls back to __str__().
_ltltoeng = None
_ltltoeng_checked = False

def _get_ltltoeng():
    global _ltltoeng, _ltltoeng_checked
    if not _ltltoeng_checked:
        try:
            import ltltoeng
            _ltltoeng = ltltoeng
        except ImportError:
            _ltltoeng = None
        _ltltoeng_checked = True
    return _ltltoeng
...
class LTLNode(ABC):
    ...
    def __to_english__(self):
        eng = _get_ltltoeng()
        if eng is not None:
            x = eng.apply_special_pattern_if_possible(self)
            if x is not None:
                return x
...
class UntilNode(BinaryOperatorNode):
    ...
    def __to_english__(self):
        ...
        if eng is not None and type(self.left) is LiteralNode and type(self.right) is LiteralNode:
            patterns = [
                f"{lhs} until {rhs}",
                f"{lhs} holds until {rhs} occurs",
                f"maintain {lhs} until {rhs}"
            ]
            return eng.choose_best_sentence(patterns)
...
class GloballyNode(UnaryOperatorNode):
    ...
    def __to_english__(self):
        ...
        eng = _get_ltltoeng()
        if eng is not None:
            english = eng.choose_best_sentence(patterns)
            return english
        return patterns[0]
...
class FinallyNode(UnaryOperatorNode):
    ...
    def __to_english__(self):
        ...
        # Provide more natural alternatives
        if eng is not None and type(self.operand) is LiteralNode:
            patterns = [
                f"eventually, {op}",
...
class ImpliesNode(BinaryOperatorNode):
    ...
    def __to_english__(self):
        ...
        if eng is not None:
            lhs = eng.normalize_embedded_clause(lhs)
            rhs = eng.normalize_embedded_clause(rhs)
